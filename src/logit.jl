function felogit(df::DataFrame, formula::FormulaTerm; vcov = Vcov.simple(), weights = nothing, kwargs...)
    ...
    if has_fes && get(kwargs, :collapse, true)
        ...
        if length(gdf) < 0.8 * length(y)
            ...
            # For logit, y_mean should be sum(y)/sum(w)
            ...
            res = felogit_fit(y_c, X_c, fes_c; weights = w_c, kwargs...)
            ...
            return PanelestModel(
                ...
                formula, coefnames_X, :logit, X_resid_full, res.XtWX
            )
        end
    end
    ...
    return PanelestModel(
        ...
        res.converged, res.iterations, length(y),
        Int(sum(w_vec)) - size(X, 2) - length(fes),
        formula, coefnames_X, :logit, res.X_resid, res.XtWX
    )
end

function felogit_fit(y::AbstractVector{<:Real}, X::AbstractMatrix{Float64}, fes::Vector{FixedEffect};
    ...
    # 1. Initialize beta
    # For logit, use logit(mean(y)) or small values
    beta = zeros(p)
    ...
    for i in 1:maxiter
        ...
        if has_fes
            mu, fe_converged, fe_iters = fe_convergence_logit(
        ...
        X_resid .= X
        if has_fes
            # Weights for Logit Hessian are w * mu * (1 - mu)
            total_w = weights .* mu .* (1.0 .- mu)
            solve_residuals_fixest!(collect(eachcol(X_resid)), fes, total_w; tol=fixef_tol, solver=solver)
        end
        
        XtWX .= X_resid' * (weights .* mu .* (1.0 .- mu) .* X_resid)
